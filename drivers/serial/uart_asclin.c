...
static inline void uart_asclin_set_bittime(Ifx_ASCLIN *base, uint8_t oversampling,
					   ...
					   bool median_filter)
{
	Ifx_ASCLIN_BITCON_Bits bitcon = {.OVERSAMPLING = oversampling - 1,
					 .PRESCALER = prescaler - 1,
					 .SAMPLEPOINT = samplepoint,
					 .SM = median_filter};

	base->BITCON.B = bitcon;
}
...
static int uart_asclin_err_check(const struct device *dev)
{
	...
	Ifx_ASCLIN_FLAGSCLEAR clr = {.B.PEC = 1, .B.FEC = 1, .B.BDC = 1, .B.RFOC = 1, .B.CEC = 1};
				     ...
				     uint8_t oversampling)
{
	...
	Ifx_ASCLIN_BRG_Bits brg = {.NUMERATOR = m[0][0], .DENOMINATOR = m[1][0]};
...
{
	Ifx_ASCLIN_FRAMECON_Bits framecon = {
		.STOP = stop_bits, .MODE = 1, .PEN = parity_enable, .ODD = parity_odd};
	Ifx_ASCLIN_DATCON_Bits datcon = {.DATLEN = data_bits};
	Ifx_ASCLIN_TXFIFOCON_Bits txfifocon = {
		.FLUSH = 1, .ENO = 1, .FM = 0, .INW = (data_bits > 8 ? 2 : 1)};
	Ifx_ASCLIN_RXFIFOCON_Bits rxfifocon = {
		.FLUSH = 1, .ENI = 1, .FM = 0, .OUTW = (data_bits > 8 ? 2 : 1)};

	/* Frame Configuration*/
	base->FRAMECON.B = framecon;
	/* Data configuration */
	base->DATCON.B = datcon;
	/* Fifo configuration */
	base->TXFIFOCON.B = txfifocon;
	base->RXFIFOCON.B = rxfifocon;
}
...
static int uart_asclin_irq_is_pending(const struct device *dev)
{
	...
	return (data->flags.B.RFL && (en & BIT(28))) || (data->flags.B.TFL && (en & BIT(31)));
}
